...
SENSITIVE_PATTERNS = [
    re.compile(r'\bGEA\b', re.IGNORECASE),          # GE Appliances
    re.compile(r'\bGE\s+APPLIANCES\b', re.IGNORECASE),
    re.compile(r'\bGENERAL\s+ELECTRIC\b', re.IGNORECASE),
    re.compile(r'(?<!\w)GE(?!\w)', re.IGNORECASE),   # 独立的 GE（不匹配 EDGE, RANGE 等）
    re.compile(r'TOXIC', re.IGNORECASE),              # TOXICITY, TOXIC
    re.compile(r'ROPER\s+CORP', re.IGNORECASE),       # 另一个可能的公司名
]


def filter_sensitive_lines(text):
    """
    过滤翻译文本中的敏感行（公司名、毒性程序等）。
    返回 (过滤后文本, 被过滤的行列表)
    """
    lines = text.split("\n")
    clean = []
    filtered = []
    for line in lines:
        is_sensitive = any(p.search(line) for p in SENSITIVE_PATTERNS)
        if is_sensitive:
...
def translate_block(block_text, block_image):
    ...
    # 翻译后再过滤一次（GPT 可能从图片里看到敏感内容）
    result, post_filtered = filter_sensitive_lines(result)
    if post_filtered:
        print(f"      翻译后过滤 {len(post_filtered)} 行")
    return result
...
def batch_translate_labels(blocks, indices, block_texts):
    ...
    # 构建输入（跳过敏感内容）
    lines = []
    valid_indices = []  # 实际参与翻译的 block 索引
    for i, bi in enumerate(indices):
        text = " ".join(it["text"] for it in blocks[bi])
        _, filtered = filter_sensitive_lines(text)
        if filtered:
            print(f"      跳过敏感标注 [{bi+1}]: {text}")
            continue
    ...
    for line in result_text.strip().split("\n"):
        ...
        if m:
            ...
            if idx < len(valid_indices):
                translations[valid_indices[idx]] = m.group(2).strip()
